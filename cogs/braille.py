        ...
        def generator(path, width, autocontrast, inverted, dither):
            ...
            def calc_average(img, algorythm, autocontrast):
                if autocontrast:
                    ...
                    for y in range(img.size[1]):
                        for x in range(img.size[0]):
                            if algorythm == "RGBsum":
                                average += img.getpixel((x, y))[0]+img.getpixel((x, y))[
                                    1]+img.getpixel((x, y))[2]
                            elif algorythm == "R":
            ...
            def iterate_image(img, original_img, dither, autocontrast, noempty, colorstyle, blank):
                ...
                while y_pos < y_size-3:
                    ...
                    while x_pos < x_size:
                        line = line + \
                            color_average_at_cursor(
                                original_img, (x_pos, y_pos), colorstyle)
                        line = line + \
                            block_from_cursor(
                                img, (x_pos, y_pos), average, noempty, blank)
            ...
            img = img.resize(
                (new_width, round((new_width*img.size[1])/img.size[0])))
            off_x = (img.size[0] % 2)
            off_y = (img.size[1] % 4)
            if off_x + off_y > 0:
            ...
            # Get your output!
            iterate_image(img, original_img, dither,
                          autocontrast, noempty, colorstyle, blank)
